                        ...
                        @foreach ($kategoris as $kategori)
                            <tr>
                                ...
                                <td class="text-center">
                                    @if (Auth::check() && (Auth::user()->isAdmin() || Auth::user()->isManager()))
                                        <button class="btn btn-warning" onclick="window.location.href='{{ route('kategori.edit', ['kategori' => $kategori->id]) }}'"><i class="fa fa-edit"></i></button>

                                        <form action="{{ route('kategori.destroy', ['kategori' => $kategori->id]) }}"
                                            method="POST" style="display:inline-block"
                                            id="delete-form-{{ $kategori->id }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-danger delete-button"
                                                data-id="{{ $kategori->id }}"><i class="fa fa-trash"></i></button>
                                        </form>
                                    @else
